...
func TestParseCSITextAreaReply(testingT *testing.T) {
	cases := []struct {
		name string
		in   string
		cols int
	...
	for _, tc := range cases {
		...
		if ok != tc.ok || cols != tc.cols || rows != tc.rows {
			testingT.Errorf("%s: got (%d,%d,%v), want (%d,%d,%v)",
...
func TestWinsizePollStops(testingT *testing.T) {
	model := tui.New(config.Default(true), "/tmp/test-gentoo.toml")
	stopped := false
	for idx := 0; idx < 12; idx++ {
		mm, cmd := model.Update(tui.WinsizeTickMsg{})
		model = mm.(*tui.Model)
		if cmd == nil {
			stopped = true
			break
	...
	if !stopped {
		testingT.Fatal("winsize poll did not stop after stable reads")
	}
}
